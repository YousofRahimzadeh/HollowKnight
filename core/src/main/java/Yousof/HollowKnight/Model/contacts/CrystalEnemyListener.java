...
public class CrystalEnemyListener implements ContactListener{
    ...
    private void aroundSensor(Fixture knightt , Fixture sensor , boolean Begin){
        if ("CrystalGuardian_knightRight_is_around".equals(sensor.getUserData()) && "Knight_main_body".equals(knightt.getUserData())) {
            Knight knight = (Knight) knightt.getBody().getUserData();
            CrystalSeeSensors sentrySensor = ((CrystalGuardian)sensor.getBody().getUserData()).getSeeSensors();
            sentrySensor.knightRight = (Begin) ? knight : null;
        }else if ("CrystalGuardian_knightLeft_is_around".equals(sensor.getUserData()) && "Knight_main_body".equals(knightt.getUserData())) {
            Knight knight = (Knight) knightt.getBody().getUserData();
            CrystalSeeSensors sentrySensor = ((CrystalGuardian)sensor.getBody().getUserData()).getSeeSensors();
    ...
    private void checkSensorContact(Fixture sensor, Fixture obstacle, boolean isBegin) {
        if ((obstacle.getUserData() != null && 
            ...
            sensor.getBody().getUserData() instanceof CrystalGuardian) {

            CrystalGuardian enemy = (CrystalGuardian) sensor.getBody().getUserData();
            String sensorData = (String) sensor.getUserData();
            
            if (sensorData.equals("CrystalGuardian_sensor_wall_right")) {
                enemy.getSurroundSensors().rightWall += (isBegin) ? 1 : -1;
            } else if (sensorData.equals("CrystalGuardian_sensor_wall_left")) {
                enemy.getSurroundSensors().leftWall += (isBegin) ? 1 : -1;
            } else if (sensorData.equals("CrystalGuardian_sensor_cliff_right")) {
                enemy.getSurroundSensors().rightCliff += (isBegin) ? 1 : -1;
                if(isBegin) enemy.getSurroundSensors().rightCliffInitialized = true;
            } else if (sensorData.equals("CrystalGuardian_sensor_cliff_left")) {
